...
//                System.out.println(Cdr.getServiceId()+"+++"+Cdr.getCdrId());
    ...
    private int getContractId(String msisdn) {
        try {
            ps = db.con.prepareStatement("select contract_id from bscs.contract where msisdn=? ");
...
private void setFreeUnits(float bucketConsumption, int contractid) {
 ...
 private void setUDRTODB(Cdr uploadCDR, int contractid) {
        try {
            ...
            PreparedStatement ps = db.con.prepareStatement("insert into rtx.udr (diala,dialb,serviceid,startdate,starttime,duration,rpid,externalcharge,contractid)"
                    + "                                                  values(?,?,?,?,?,?,?,?,?)");
    ...
    private void changeCDRIsRationg(int cdrId) {
        try {
            PreparedStatement ps = db.con.prepareStatement("update public.cdr set israting=true where id=? ");
            ps.setInt(1, cdrId);
            int done = ps.executeUpdate();
